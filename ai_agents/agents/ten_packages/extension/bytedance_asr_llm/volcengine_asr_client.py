...
class ASRResponse:
    ...
    utterances: list[Utterance] = None  # Recognition result speech segmentation information
...
class RequestBuilder:
    ...
    @staticmethod
    def new_auth_headers(app_key: str, access_key: str, resource_id: str = "volc.bigasr.sauc.duration") -> Dict[str, str]:
    ...
    def new_full_client_request(seq: int, config: BytedanceASRLLMConfig) -> bytes:
        ...
        header = ASRRequestHeader.default_header() \
            .with_message_type_specific_flags(MESSAGE_TYPE_SPECIFIC_FLAGS_POS_SEQUENCE)
    ...
    def new_audio_only_request(seq: int, segment: bytes, is_last: bool = False) -> bytes:
        ...
        header = ASRRequestHeader.default_header()
        if is_last:
            header.with_message_type_specific_flags(MESSAGE_TYPE_SPECIFIC_FLAGS_NEG_WITH_SEQUENCE)
            seq = -seq
        else:
            header.with_message_type_specific_flags(MESSAGE_TYPE_SPECIFIC_FLAGS_POS_SEQUENCE)
...
class ResponseParser:
    ...
    @staticmethod
    def parse_response(msg: bytes, config: Optional['BytedanceASRLLMConfig'] = None) -> ASRResponse:
        ...
        if message_type == MESSAGE_TYPE_SERVER_FULL_RESPONSE:
            ...
            if len(payload) >= 8:
                response.code = struct.unpack('>i', payload[:4])[0]  # Error message code
                error_message_size = struct.unpack('>I', payload[4:8])[0]  # Error message size
                payload = payload[8:]
                ...
                if error_message_size > 0 and len(payload) >= error_message_size:
                    try:
                        ...
                        response.payload_msg = {"error_message": str(payload[:error_message_size])}
        ...
        # Parse payload
        try:
            if serialization_method == SERIALIZATION_TYPE_JSON:
                ...
                if isinstance(payload_data, dict):
                    ...
                    if result_data and isinstance(result_data, dict):
                        ...
                        if response.utterances:
                            ...
                            if "audio_info" in payload_data and "duration" in payload_data["audio_info"]:
                                response.duration_ms = payload_data["audio_info"]["duration"]
                            else:
                                # Fallback: calculate from first utterance
                                response.duration_ms = first_utt.end_time - first_utt.start_time
                    else:
...
class VolcengineASRClient:
    ...
    def __init__(self, url: str, app_key: str, access_key: str, config: BytedanceASRLLMConfig, ten_env=None):
        self.url = url
    ...
    def _calculate_segment_size(self) -> int:
        ...
        bytes_per_sec = (self.config.bits // 8) * self.config.channel * self.config.sample_rate
    ...
    async def connect(self) -> None:
        ...
        headers = RequestBuilder.new_auth_headers(self.app_key, self.access_key, self.config.resource_id)
        try:
            ...
            if hasattr(self.websocket, 'response_headers'):
                ...
                if tt_logid:
                    if self.ten_env:
                        self.ten_env.log_info(f"WebSocket connected with X-Tt-Logid: {tt_logid}")
                    else:
                        logging.info(f"WebSocket connected with X-Tt-Logid: {tt_logid}")
            ...
            if self.connection_error_callback:
                try:
                    ...
                    if self.ten_env:
                        self.ten_env.log_error(f"Error in connection error callback: {callback_error}")
                    else:
                        logging.error(f"Error in connection error callback: {callback_error}")
            else:
    ...
    async def _send_full_client_request(self) -> None:
        ...
        logging.info(f"Sent full client request with seq: {self.seq-1}")
    ...
    async def finalize(self) -> None:
        ...
        if self.audio_buffer:
            await self._send_audio_segment(bytes(self.audio_buffer), True)
    ...
    async def _listen_for_responses(self) -> None:
        ...
        try:
            ...
            if self.ten_env:
                self.ten_env.log(LogLevel.ERROR, f"Error listening for responses: {e}")
            else:
            ...
            if self.asr_error_callback:
                try:
                    ...
                    if self.ten_env:
                        self.ten_env.log(LogLevel.ERROR, f"Error in ASR error callback: {callback_error}")
                    else:
    ...
    async def _handle_response(self, response: ASRResponse) -> None:
        ...
        if self.result_callback:
            try:
                ...
                self.ten_env.log_error(LogLevel.ERROR, f"Error in result callback: {e}")
        else:
            self.ten_env.log_warn(LogLevel.WARNING, "result_callback is not set")

    def set_on_connection_error_callback(self, callback: Callable[[Exception], None]) -> None:
    ...
    def set_on_result_callback(self, callback: Callable[[ASRResponse], Awaitable[None]]) -> None:
