...
RECONNECTABLE_ERROR_CODES = {
    ...
    VOLCENGINE_ERROR_CODES["PACKET_TIMEOUT"],      # Packet timeout - network issue, retryable
    VOLCENGINE_ERROR_CODES["INVALID_AUDIO_FORMAT"], # Invalid audio format - format issue, retryable
    VOLCENGINE_ERROR_CODES["INTERNAL_ERROR"],      # Internal error - server issue, retryable
    VOLCENGINE_ERROR_CODES["SERVER_BUSY"],         # Server busy - temporary overload, retryable
}

# Fatal error codes - errors that should not be retried
FATAL_ERROR_CODES = {
    VOLCENGINE_ERROR_CODES["INVALID_PARAMETER"],   # Invalid parameters - configuration issue, retry won't help
}
